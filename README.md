...
- **Measurement tools** — interactive on-canvas distance (two-point ruler), area (N-point polygon with live area + perimeter), and angle (3-point, directed `[0°, 360°)` with reflex), selected via `v-model:measure-mode`; backed by pure `measureDistance` / `measureArea` / `measurePerimeter` / `polygonSelfIntersects` / `measureAngle` / `measureDirectedAngle` utilities (2D/3D, Shoelace polygon area, self-intersection test, unsigned + directed angles)
