...
export function createSnapController(
  /**
   * Optional callback fired when the active snap type changes (including to
   * `null` when the marker hides). Framework wrappers use this to mirror the
   * value into a reactive ref; omit it for a purely imperative consumer.
   */
  onSnapChange?: (type: SnapType | null) => void,
): SnapController {
  /** The kind of snap currently shown under the cursor, or `null`. */
  let current: SnapType | null = null;

  /** Update `current` and notify the optional reactive bridge on change. */
  const setCurrent = (next: SnapType | null): void => {
