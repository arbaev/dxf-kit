...
import { useThreeScene, type ThreeJSOptions, type ThreeSceneController } from "./useThreeScene";
 ...
 * React state via setter callbacks. `isLoading` is owned by the component (the
 * load functions live there), so it is not driven from here.
 */
function createDXFRendererController(
  ...
  // Push the debug snapshot to React only when it actually changed — otherwise
  // React would re-render the whole tree on every frame. zoomPercent is a
  // primitive, so React bails out on equality.
...
/**
 * React hook over {@link createDXFRendererController}. Composes
 * {@link useThreeScene} and surfaces the renderer's reactive state
 * (`isLoading` + `displayProgress` + `zoomPercent` + `debugInfo`) alongside the
 * scene's `webGLSupported` / `error`. `setIsLoading` is returned so the owning
 * component can drive the loading flag from its load functions.
 */
export function useDXFRenderer() {
  ...
  const ref = useRef<DXFRendererController | null>(null);
  if (ref.current === null) {
    ref.current = createDXFRendererController(three.controller, {
      setDisplayProgress,
      setZoomPercent,
      setDebugInfo,
    });
  ...
  return {
    ...
    zoomPercent,
    debugInfo,
    webGLSupported: three.webGLSupported,
    error: three.error,

    controller,
    initThreeJS: controller.initThreeJS,
    parseDXF: controller.parseDXF,
    parseDXFAsync: controller.parseDXFAsync,
    displayDXF: controller.displayDXF,
    handleResize: controller.handleResize,
    resetView: controller.resetView,
    zoomToBox: controller.zoomToBox,
    applyLayerVisibility: controller.applyLayerVisibility,
    ...
    getOriginOffset: controller.getOriginOffset,
