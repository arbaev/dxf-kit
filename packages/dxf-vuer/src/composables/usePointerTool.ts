 ...
 * measurement). It owns everything the three tools used to re-implement
 * independently: the capture-phase pointer pipeline, the click-vs-pan
 * threshold, the LEFT-mouse-button steal/restore (so a left drag measures
 * instead of panning), the overlay-group lifecycle, and screen↔world
 * projection. A tool plugs its state machine in via {@link PointerToolHooks}
 * and keeps only its own overlay geometry + result logic.
 *
 * The pure projection/geometry helpers are exported standalone so they can be
 * unit-tested without a DOM and reused 1:1 by future React / Lit ports.
 */
...
/**
 * Create a pointer-tool foundation. Call {@link configure} once (synchronously,
 * during composable setup) with the tool's hooks, then expose the base's
 * `attach` / `detach` / `setEnabled` / `setColor` / `dispose` from the tool.
 */
export function usePointerTool<C extends PointerToolBaseCallbacks>() {
  ...
  const applySnap = (
    ...
    clientX: number,
    clientY: number,
  ): MeasurePoint => (callbacks.snap ? callbacks.snap(raw, clientX, clientY) : raw);
  ...
  return {
    isActive,
    /** Register the tool's hooks. Call once, synchronously, before `attach`. */
    ...
    setColor,
    dispose,
    // --- primitives the tool's state machine builds on ---
    ensureOverlay,
    screenToWorld,
    worldToScreen,
