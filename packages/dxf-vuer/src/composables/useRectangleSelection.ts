 ...
 * just the `enabled` flag. Decoupled so the composable can be unit-tested
 * with a plain `{ enabled: true }` object.
 */
...
export interface RectSelectionCallbacks {
  ...
  /** Final entry list at pointerup. Empty array is valid (user dragged over empty space). */
...
/**
 * Build a world-space rectangle from two screen points. The composable adds
 * `originOffset` back so the rect lives in the same DXF world coords that
 * `PickingEntry.bbox` does. Pure — exported for testing.
 */
...
/**
 * Drag-state machine for rectangle (window/crossing) selection.
 *
 * Flow:
 *   1. User holds the modifier key (Shift by default) → cursor turns into
 ...
 *   5. Esc during a drag fires `onCancel` + `onEnd` and leaves selection
 *      untouched.
 *
 * `screenRect` and `isDragging` are reactive so the consumer can render the
 * visual overlay declaratively.
 */
export function useRectangleSelection() {
  ...
  /**
   * Snapshot of `controls.enabled` taken the first time we disabled it
   * (either on modifier-keydown or on pointerdown). Restored exactly once
   * when both the modifier is released AND no drag is in flight.
   * `null` means "we have not stolen control".
   */
  ...
  const handlePointerDown = (e: PointerEvent): void => {
    ...
    acquireControls();
    // Deliberately NOT stopping propagation — `usePicking` shares the same
    // 4px click threshold, so any drag long enough for us to act on is also
    // long enough for picking to suppress its own `entity-click` emission.
    // Letting picking see pointerdown keeps its mousedownX/Y in sync with
    // the matching pointerup that follows.
  };
  ...
  const updateScreenRect = (endX: number, endY: number): void => {
    ...
    screenRect.value = normaliseScreenRect(
      startScreen,
      { x: endX, y: endY },
  ...
  return {
    ...
    isDragging,
    /** Canvas-relative rectangle for visual overlay. `null` when idle. */
    screenRect,
    attach,
    detach,
    installRectData,
