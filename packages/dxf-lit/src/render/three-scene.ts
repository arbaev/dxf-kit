...
/**
 * Plain (framework-agnostic) controller holding the Three.js scene lifecycle.
 * `webGLSupported` / `error` are surfaced through setter callbacks the host
 * (the Lit element) bridges into its reactive `@state`. All mutable state lives
 * here (created once, held on the element instance), so it survives re-renders.
 *
 * Ported verbatim from dxf-react's `createThreeSceneController` — the only
 * framework-coupled piece (the `useThreeScene` hook) is omitted; the Lit
 * element wires the setters instead.
 */
export function createThreeSceneController(setters: ThreeSceneSetters): ThreeSceneController {
